# Cortex — Plan d'implémentation technique
...
- **UI** → dépôt console `Atlas Studio Console Admin` : `src/admin/pages/cortex/` (calqué sur `asvc/`).
- **DB + Edge Functions** → dépôt site `Atlas-Studio-Website/supabase/` : migrations `cps_*` et fonctions `cortex-*` (même Supabase que la console).
...
| # | Sujet | Constat | Reco |
|---|-------|---------|------|
| D1 | **Accent ambre `#EF9F27`** | La console est en **olive `#6C7A1E` + volt `#C4EC00`** (palette globale). L'ambre du CDC n'existe pas et jurerait si on touche `--c-accent`. | **Scoper l'ambre à Cortex** via `data-module="cortex"` (variables locales `--cps-accent`…), sans toucher la palette globale. Cortex aura son identité ambre, le reste reste olive/volt. |
...
- **Module UI** : `src/admin/pages/cortex/` = pages `CPS-*.tsx` + `hooks.ts` (hooks `useState/useEffect`, pas de React Query) + `types.ts` + `components/`.
- **Router** (`src/main.tsx`) : `lazyRetry(() => import('./admin/pages/cortex/…'))` + `S(<Page/>)` (Suspense), routes sous `/admin` dans `<RequireAdmin><AdminLayout>`.
- **Nav** (`src/admin/AdminSidebar.tsx`) : une entrée `NAV_GROUPS` `{ id:"cortex", label:"Cortex", icon, subgroups:[…] }`.
...
- `cortex-ingest` (V3) — réception événements apps, **HMAC-SHA256** par source (réutilise `_shared/federation_auth.ts`), idempotence.
- `cortex-aggregate` (V2/V3) — MRR, burn, runway, effort, vélocité → `cps_metrics_*` (pg_cron).
- `cortex-project` (V2) — projections scénarisées, `inputs_hash`.
- `cortex-advisor-feed` (V4) — sérialise agrégats → `cpsChat()` → insère dans `cps_proph3t_insights` via rôle `proph3t_writer` ; **post-contrôle anti-hallucination** (tout nombre cité doit exister dans les inputs, sinon rejet `hallucination_check_failed`).
- pg_cron : agrégations + analyses PROPH3T quotidienne/hebdo/mensuelle.

---

## 5. Écrans (module `cortex/`)
...
| **V2** | Finance : pricing, coûts, `cortex-project`, scénarios, CPS-30/31, export XLSX. | Réalisé vs projeté, burn/runway/point mort. |
| **V3** | Data Fabric temps réel : `cortex-ingest` + HMAC + SDK `@atlas/cortex-emitter`, 1re app instrumentée, Realtime, CPS-90. | Dashboard live < 5 s. |
| **V4** | PROPH3T Advisor : `cortex-advisor-feed`, `proph3t_writer`, analyses programmées, anti-hallucination, CPS-60. | Insights advisory validables. |
...
Une fois D1–D4 tranchés : je crée la **branche Supabase**, j'écris **`cps_foundation.sql`** (tables Vague 1 + audit + RLS), je la teste sur la branche (insert + vérif chaîne audit), puis je monte l'ossature UI `cortex/` (nav + routes + CPS-00/10/11 avec saisie manuelle). Build + revue avant tout merge prod.
