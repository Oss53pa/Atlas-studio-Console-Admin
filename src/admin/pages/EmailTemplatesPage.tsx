...
const HEADER = `<tr><td style="background:linear-gradient(135deg,var(--c-surface) 0%,var(--c-surface) 50%,var(--c-surface) 100%);padding:36px 40px;text-align:center;"><div style="font-size:28px;font-weight:800;color:#ffffff;letter-spacing:2px;">ATLAS <span style="color:#C0635C;">STUDIO</span></div><div style="font-size:12px;color:#B9B7B1;margin-top:4px;letter-spacing:1px;">atlas-studio.org</div></td></tr>`;

const FOOTER = (extra: string) => `<tr><td style="background:#F5F5F4;border-top:1px solid #E9E8E6;padding:20px 40px;text-align:center;"><p style="margin:0;font-size:12px;color:#B9B7B1;">&copy; Atlas Studio | <a href="https://atlas-studio.org" style="color:#B9B7B1;text-decoration:none;">atlas-studio.org</a> | <a href="mailto:support@atlas-studio.org" style="color:#B9B7B1;text-decoration:none;">support@atlas-studio.org</a></p><p style="margin:6px 0 0;font-size:11px;color:#D6D5D1;">${extra}</p></td></tr>`;

const wrap = (body: string) => `<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"></head><body style="margin:0;padding:0;background-color:#F5F5F4;font-family:'Segoe UI',Arial,sans-serif;"><table width="100%" cellpadding="0" cellspacing="0" style="background:#F5F5F4;padding:40px 0;"><tr><td align="center"><table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.08);max-width:600px;width:100%;">${body}</table></td></tr></table></body></html>`;

const btn = (url: string, label: string) => `<div style="text-align:center;margin:32px 0;"><a href="${url}" style="background:linear-gradient(135deg,#C0635C,#c0392b);color:#ffffff;text-decoration:none;padding:14px 36px;border-radius:8px;font-size:15px;font-weight:700;display:inline-block;letter-spacing:0.5px;">${label}</a></div>`;

const infoBox = (content: string) => `<div style="background:#F5F5F4;border-left:4px solid #C0635C;border-radius:6px;padding:16px 20px;margin:0 0 28px;">${content}</div>`;
...
function welcomeTenantHtml(p: Record<string, string>) {
  return wrap(`${HEADER}<tr><td style="padding:40px 40px 32px;"><p style="font-size:22px;font-weight:700;color:var(--c-surface);margin:0 0 8px;">Bienvenue, ${p.admin_first_name} !</p><p style="font-size:15px;color:#4C4A46;margin:0 0 24px;line-height:1.7;">Votre espace <strong>${p.company_name}</strong> sur <strong>${p.app_name}</strong> est pr&ecirc;t. Vous pouvez d&egrave;s maintenant acc&eacute;der &agrave; votre workspace.</p>${infoBox(`<p style="margin:0;font-size:13px;color:#8E8C86;">Application souscrite : <strong style="color:var(--c-surface);">${p.app_name}</strong></p><p style="margin:8px 0 0;font-size:13px;color:#8E8C86;">Vos identifiants de connexion vous seront envoy&eacute;s dans un email s&eacute;par&eacute;.</p>`)}${btn(p.workspace_url, 'Acc&eacute;der &agrave; mon workspace')}<p style="font-size:13px;color:#B9B7B1;text-align:center;margin:0;">Des questions ? <a href="mailto:${p.support_email}" style="color:#C0635C;text-decoration:none;">${p.support_email}</a></p></td></tr>${FOOTER(`Cet email a &eacute;t&eacute; envoy&eacute; par Atlas Studio pour le compte de ${p.company_name}.`)}`);
}

function welcomeUserHtml(p: Record<string, string>) {
  return wrap(`${HEADER}<tr><td style="padding:40px 40px 32px;"><p style="font-size:22px;font-weight:700;color:var(--c-surface);margin:0 0 8px;">Bonjour ${p.first_name} !</p><p style="font-size:15px;color:#4C4A46;margin:0 0 24px;line-height:1.7;"><strong>${p.admin_first_name}</strong> vous invite &agrave; rejoindre l'espace <strong>${p.company_name}</strong> sur <strong>${p.app_name}</strong>.</p><div style="background:#F5F5F4;border-radius:8px;padding:20px 24px;margin:0 0 28px;border:1px solid #E9E8E6;"><p style="margin:0 0 8px;font-size:13px;color:#8E8C86;">R&ocirc;le attribu&eacute; : <strong style="color:var(--c-surface);">${p.role}</strong></p><p style="margin:0 0 8px;font-size:13px;color:#8E8C86;">Email : <strong style="color:var(--c-surface);">${p.email}</strong></p><p style="margin:0;font-size:13px;color:#8E8C86;">Mot de passe temporaire : <strong style="color:#C0635C;font-family:monospace;font-size:14px;">${p.temp_password}</strong></p></div><div style="background:#fffbeb;border:1px solid #f6e05e;border-radius:6px;padding:12px 16px;margin:0 0 28px;"><p style="margin:0;font-size:13px;color:#744210;">Pour votre s&eacute;curit&eacute;, veuillez changer votre mot de passe d&egrave;s votre premi&egrave;re connexion.</p></div>${btn(p.login_url, 'Me connecter')}<p style="font-size:13px;color:#B9B7B1;text-align:center;margin:0;">Des questions ? <a href="mailto:support@atlas-studio.org" style="color:#C0635C;text-decoration:none;">support@atlas-studio.org</a></p></td></tr>${FOOTER(`Cet email a &eacute;t&eacute; envoy&eacute; par Atlas Studio pour le compte de ${p.company_name}.`)}`);
}

function resetPasswordHtml(p: Record<string, string>) {
  return wrap(`${HEADER}<tr><td style="padding:40px 40px 32px;"><p style="font-size:22px;font-weight:700;color:var(--c-surface);margin:0 0 8px;text-align:center;">R&eacute;initialisation du mot de passe</p><p style="font-size:15px;color:#4C4A46;margin:0 0 24px;line-height:1.7;text-align:center;">Bonjour <strong>${p.first_name}</strong>, nous avons re&ccedil;u une demande de r&eacute;initialisation de votre mot de passe Atlas Studio.</p>${btn(p.reset_url, 'R&eacute;initialiser mon mot de passe')}<div style="background:#fff5f5;border:1px solid #feb2b2;border-radius:6px;padding:12px 16px;margin:0 0 20px;"><p style="margin:0;font-size:13px;color:#c53030;text-align:center;">Ce lien est valable <strong>${p.expires_in}</strong> uniquement.</p></div><div style="background:#F5F5F4;border-radius:6px;padding:12px 16px;"><p style="margin:0;font-size:13px;color:#8E8C86;text-align:center;">Si vous n'&ecirc;tes pas &agrave; l'origine de cette demande, ignorez simplement cet email.</p></div></td></tr>${FOOTER('Cet email a &eacute;t&eacute; envoy&eacute; automatiquement par Atlas Studio.')}`);
}

function confirmSubscriptionHtml(p: Record<string, string>) {
  return wrap(`${HEADER}<tr><td style="padding:40px 40px 32px;"><p style="font-size:22px;font-weight:700;color:var(--c-surface);margin:0 0 8px;">Souscription confirm&eacute;e</p><p style="font-size:15px;color:#4C4A46;margin:0 0 28px;line-height:1.7;">Merci <strong>${p.company_name}</strong>, votre souscription a bien &eacute;t&eacute; enregistr&eacute;e.</p><table width="100%" cellpadding="0" cellspacing="0" style="background:#F5F5F4;border-radius:8px;overflow:hidden;border:1px solid #E9E8E6;margin-bottom:28px;"><tr><td colspan="2" style="background:var(--c-surface);padding:12px 20px;"><p style="margin:0;font-size:13px;font-weight:700;color:#ffffff;letter-spacing:0.5px;">R&Eacute;CAPITULATIF DE SOUSCRIPTION</p></td></tr><tr><td style="padding:12px 20px;border-bottom:1px solid #E9E8E6;width:50%;"><p style="margin:0;font-size:12px;color:#8E8C86;">Application</p><p style="margin:4px 0 0;font-size:14px;font-weight:700;color:var(--c-surface);">${p.app_name}</p></td><td style="padding:12px 20px;border-bottom:1px solid #E9E8E6;"><p style="margin:0;font-size:12px;color:#8E8C86;">Plan</p><p style="margin:4px 0 0;font-size:14px;font-weight:700;color:#C0635C;">${p.plan_name}</p></td></tr><tr><td style="padding:12px 20px;"><p style="margin:0;font-size:12px;color:#8E8C86;">Montant</p><p style="margin:4px 0 0;font-size:14px;font-weight:700;color:var(--c-surface);">${p.amount} ${p.currency} / ${p.billing_period}</p></td><td style="padding:12px 20px;"><p style="margin:0;font-size:12px;color:#8E8C86;">Prochaine &eacute;ch&eacute;ance</p><p style="margin:4px 0 0;font-size:14px;font-weight:700;color:var(--c-surface);">${p.next_billing_date}</p></td></tr></table>${btn(p.invoice_url, 'T&eacute;l&eacute;charger ma facture')}<p style="font-size:13px;color:#B9B7B1;text-align:center;margin:0;">Questions ? <a href="mailto:billing@atlas-studio.org" style="color:#C0635C;text-decoration:none;">billing@atlas-studio.org</a></p></td></tr>${FOOTER(`Cet email a &eacute;t&eacute; envoy&eacute; par Atlas Studio pour le compte de ${p.company_name}.`)}`);
}

function notificationGenericHtml(p: Record<string, string>) {
  return wrap(`${HEADER.replace('atlas-studio.org', `${p.app_name} &middot; atlas-studio.org`)}<tr><td style="padding:40px 40px 32px;"><p style="font-size:20px;font-weight:700;color:var(--c-surface);margin:0 0 6px;">${p.notification_title}</p><p style="font-size:13px;color:#B9B7B1;margin:0 0 24px;">${p.company_name} &middot; ${p.app_name}</p><div style="background:#F5F5F4;border-radius:8px;padding:20px 24px;margin:0 0 28px;border:1px solid #E9E8E6;font-size:15px;color:#4C4A46;line-height:1.8;">${p.notification_body}</div>${btn(p.cta_url, p.cta_label)}<p style="font-size:13px;color:#B9B7B1;text-align:center;margin:0;">Des questions ? <a href="mailto:support@atlas-studio.org" style="color:#C0635C;text-decoration:none;">support@atlas-studio.org</a></p></td></tr>${FOOTER(`Cet email a &eacute;t&eacute; envoy&eacute; par Atlas Studio pour le compte de ${p.company_name}.`)}`);
}

function grantTestAccessHtml(p: Record<string, string>) {
  return wrap(`${HEADER}<tr><td style="padding:40px 40px 32px;"><p style="font-size:22px;font-weight:700;color:var(--c-surface);margin:0 0 8px;">Bonjour ${p.first_name} !</p><p style="font-size:15px;color:#4C4A46;margin:0 0 24px;line-height:1.7;">Bonne nouvelle ! Un acc&egrave;s test temporaire vous a &eacute;t&eacute; accord&eacute; sur <strong>${p.app_name}</strong>.</p><div style="background:#f0fdf4;border:1px solid #86efac;border-radius:8px;padding:20px 24px;margin:0 0 28px;"><p style="margin:0 0 10px;font-size:14px;color:#166534;font-weight:700;">D&eacute;tails de votre acc&egrave;s test</p><table cellpadding="0" cellspacing="0" style="width:100%;"><tr><td style="padding:4px 0;font-size:13px;color:#8E8C86;width:140px;">Application :</td><td style="padding:4px 0;font-size:13px;font-weight:700;color:var(--c-surface);">${p.app_name}</td></tr><tr><td style="padding:4px 0;font-size:13px;color:#8E8C86;">Dur&eacute;e :</td><td style="padding:4px 0;font-size:13px;font-weight:700;color:#C0635C;">${p.duration_days} jours</td></tr><tr><td style="padding:4px 0;font-size:13px;color:#8E8C86;">Expire le :</td><td style="padding:4px 0;font-size:13px;font-weight:700;color:var(--c-surface);">${p.expires_at}</td></tr><tr><td style="padding:4px 0;font-size:13px;color:#8E8C86;">Acc&egrave;s :</td><td style="padding:4px 0;font-size:13px;"><a href="${p.app_url}" style="color:#C0635C;text-decoration:none;font-weight:600;">${p.app_url}</a></td></tr></table></div><p style="font-size:15px;color:#4C4A46;margin:0 0 24px;line-height:1.7;">Profitez de cette p&eacute;riode pour explorer toutes les fonctionnalit&eacute;s. &Agrave; l'issue de cette p&eacute;riode, votre acc&egrave;s sera automatiquement d&eacute;sactiv&eacute;.</p>${btn(p.app_url, `Acc&eacute;der &agrave; ${p.app_name}`)}<div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:6px;padding:12px 16px;margin:0 0 20px;"><p style="margin:0;font-size:13px;color:#1e40af;"><strong>Astuce :</strong> Installez l'app sur votre appareil pour un acc&egrave;s rapide ! Chrome/Edge : ic&ocirc;ne &laquo; Installer &raquo; &middot; Mobile : Partager &rarr; &laquo; Sur l'&eacute;cran d'accueil &raquo;</p></div><p style="font-size:13px;color:#B9B7B1;text-align:center;margin:0;">Des questions ? <a href="mailto:support@atlas-studio.org" style="color:#C0635C;text-decoration:none;">support@atlas-studio.org</a></p></td></tr>${FOOTER(`Cet email a &eacute;t&eacute; envoy&eacute; par Atlas Studio &mdash; Acc&egrave;s test temporaire.`)}`);
}

function accountSuspendedHtml(p: Record<string, string>) {
  return wrap(`<tr><td style="background:linear-gradient(135deg,var(--c-surface) 0%,var(--c-surface) 50%,#742a2a 100%);padding:36px 40px;text-align:center;"><div style="font-size:28px;font-weight:800;color:#ffffff;letter-spacing:2px;">ATLAS <span style="color:#C0635C;">STUDIO</span></div><div style="font-size:12px;color:#B9B7B1;margin-top:4px;letter-spacing:1px;">atlas-studio.org</div></td></tr><tr><td style="padding:40px 40px 32px;"><p style="font-size:22px;font-weight:700;color:#c53030;margin:0 0 8px;text-align:center;">Acc&egrave;s suspendu</p><p style="font-size:15px;color:#4C4A46;margin:0 0 24px;line-height:1.7;text-align:center;">L'acc&egrave;s de <strong>${p.company_name}</strong> &agrave; <strong>${p.app_name}</strong> a &eacute;t&eacute; temporairement suspendu.</p><div style="background:#fff5f5;border:1px solid #feb2b2;border-radius:8px;padding:16px 20px;margin:0 0 28px;"><p style="margin:0;font-size:13px;color:#c53030;font-weight:600;">Motif :</p><p style="margin:8px 0 0;font-size:14px;color:#742a2a;">${p.reason}</p></div><p style="font-size:15px;color:#4C4A46;margin:0 0 28px;line-height:1.7;text-align:center;">Pour r&eacute;activer votre acc&egrave;s, veuillez r&eacute;gulariser votre situation.</p>${btn(p.reactivation_url, 'R&eacute;gulariser mon compte')}<p style="font-size:13px;color:#B9B7B1;text-align:center;margin:0;">Besoin d'aide ? <a href="mailto:${p.support_email}" style="color:#C0635C;text-decoration:none;">${p.support_email}</a></p></td></tr>${FOOTER(`Cet email a &eacute;t&eacute; envoy&eacute; par Atlas Studio pour le compte de ${p.company_name}.`)}`);
}
...
const SAMPLE_BASE = {
  ...
  app_name: "Atlas F&A",
  support_email: "support@atlas-studio.org",
};
...
const templates: TemplateDefinition[] = [
  {
    ...
    sender: "notifications@atlas-studio.org",
    type: "Bienvenue",
    typeColor: "#16a34a",
    ...
    samplePayload: { ...SAMPLE_BASE, workspace_url: "https://atlas-fna.atlas-studio.org" },
  ...
  },
  {
    ...
    samplePayload: { ...SAMPLE_BASE, first_name: "Fatou", role: "Comptable", login_url: "https://atlas-fna.atlas-studio.org/login", temp_password: "Tmp-8xK2m!", email: "fatou.konate@sanogo.ci" },
  ...
  },
  {
    ...
    sender: "noreply@atlas-studio.org",
    type: "Sécurité",
    typeColor: "#dc2626",
    ...
    samplePayload: { first_name: "Amadou", reset_url: "https://atlas-studio.org/reset?token=abc123", expires_in: "30 minutes" },
  ...
  },
  {
    ...
    sender: "billing@atlas-studio.org",
    type: "Facturation",
    typeColor: "var(--c-accent-dark)",
    ...
    samplePayload: { ...SAMPLE_BASE, plan_name: "PME / TPE", amount: "49 000", currency: "FCFA", billing_period: "mois", next_billing_date: "29 avril 2026", invoice_url: "https://atlas-studio.org/invoices/INV-2026-0042" },
  ...
  },
  {
    ...
    samplePayload: { ...SAMPLE_BASE, first_name: "Amadou", app_name: "Atlas F&A", duration_days: "7", expires_at: "6 avril 2026", app_url: "https://atlas-fna.atlas-studio.org" },
  ...
  },
  {
    ...
    samplePayload: { ...SAMPLE_BASE, reason: "Facture INV-2026-0038 impayée depuis 30 jours (49 000 FCFA).", reactivation_url: "https://atlas-studio.org/portal/billing", support_email: "support@atlas-studio.org" },
    ...
    <div>
      <div className="flex items-center justify-between mb-7">
        <div>
          ...
          <p className="text-neutral-muted dark:text-admin-muted text-sm">
            {templates.length} templates &middot; Envoi via Resend &middot; Domaine atlas-studio.org
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {templates.map((tpl) => (
          <div
            key={tpl.id}
            className="bg-white dark:bg-admin-surface border border-warm-border dark:border-white/5 rounded-2xl shadow-sm dark:shadow-premium p-6 flex flex-col hover:-translate-y-0.5 hover:border-gold/40 dark:hover:border-admin-accent/40 hover:shadow-md dark:hover:shadow-elev-3 transition-all duration-300"
          >
