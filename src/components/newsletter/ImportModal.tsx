...
export function ImportModal({ onClose, onImported }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
      <div className="bg-p-surface border border-p-border rounded-2xl w-full max-w-lg p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-[#F5F5F5]">Importer des abonnés</h2>
          <button onClick={onClose} className="p-1 rounded hover:bg-p-surface text-[#888]"><X size={18} /></button>
        </div>
        ...
        {result ? (
          ...
          <label className="flex flex-col items-center gap-3 p-10 border-2 border-dashed border-p-border rounded-xl cursor-pointer hover:border-p-accent/50 transition-colors">
          ...
          <>
            ...
            {preview.length > 0 && (
              <div className="overflow-x-auto mb-4 bg-p-surface rounded-lg p-3">
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div>
                ...
                <select value={mapping.email} onChange={e => setMapping({ ...mapping, email: e.target.value })}
                  className="w-full px-2 py-1.5 bg-p-surface rounded text-xs text-[#F5F5F5] outline-none">
            ...
            <button onClick={runImport} disabled={isImporting}
              className="w-full py-2.5 bg-p-accent text-black rounded-lg text-sm font-medium hover:bg-[#C47E00] disabled:opacity-50 flex items-center justify-center gap-2">
