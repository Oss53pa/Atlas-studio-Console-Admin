...
export function ImportModal({ onClose, onImported }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
      <div className="bg-[#1c1c20] border border-[#2a2a30] rounded-2xl w-full max-w-lg p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-[#F5F5F5]">Importer des abonnés</h2>
          <button onClick={onClose} className="p-1 rounded hover:bg-[#2a2a30] text-[#888]"><X size={18} /></button>
        </div>
        ...
        {result ? (
          <div className="text-center py-8">
            ...
            <button onClick={onClose} className="mt-4 px-6 py-2 bg-[#A9B57E] text-black rounded-lg text-sm font-medium">Fermer</button>
          </div>
        ) : !file ? (
          <label className="flex flex-col items-center gap-3 p-10 border-2 border-dashed border-[#2a2a30] rounded-xl cursor-pointer hover:border-[#A9B57E]/50 transition-colors">
          ...
          <>
            ...
            {preview.length > 0 && (
              <div className="overflow-x-auto mb-4 bg-[#131316] rounded-lg p-3">
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div>
                ...
                <select value={mapping.email} onChange={e => setMapping({ ...mapping, email: e.target.value })}
                  className="w-full px-2 py-1.5 bg-[#2a2a30] rounded text-xs text-[#F5F5F5] outline-none">
            ...
            <button onClick={runImport} disabled={isImporting}
              className="w-full py-2.5 bg-[#A9B57E] text-black rounded-lg text-sm font-medium hover:bg-[#C47E00] disabled:opacity-50 flex items-center justify-center gap-2">
