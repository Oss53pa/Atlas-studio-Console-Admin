...
export function CampaignStats({ campaignId, onBack }: Props) {
  ...
  if (loading || !campaign) return (
    <div className="flex items-center justify-center h-full"><Loader2 size={24} className="animate-spin text-[#A9B57E]" /></div>
  )

  const kpis = [
    { label: 'Taux d\'ouverture', value: `${openRate.toFixed(1)}%`, sub: `${campaign.unique_open_count} / ${campaign.delivered_count}`, icon: Eye, color: '#A9B57E' },
  ...
  return (
    <div className="p-6 overflow-auto">
      <div className="flex items-center gap-3 mb-6">
        <button onClick={onBack} className="p-2 rounded-lg hover:bg-[#2a2a30] text-[#888] hover:text-[#F5F5F5]"><ArrowLeft size={18} /></button>
        <div>
        ...
        <div className="bg-[#1c1c20] border border-[#2a2a30] rounded-xl p-5">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={hourData}>
              ...
              <Tooltip contentStyle={{ background: '#1c1c20', border: '1px solid #2a2a30', borderRadius: 8, fontSize: 12 }} />
              <Bar dataKey="count" fill="#A9B57E" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
        <div className="bg-[#1c1c20] border border-[#2a2a30] rounded-xl p-5">
      ...
      {campaign.ab_test_enabled && (
        <div className="mt-6 bg-[#1c1c20] border border-[#2a2a30] rounded-xl p-5">
          <h3 className="text-sm font-medium text-[#F5F5F5] mb-4">Résultats A/B Test</h3>
          <div className="grid grid-cols-2 gap-4">
            <div className="p-4 rounded-lg bg-[#2a2a30]/30">
              <div className="text-xs text-[#A9B57E] font-medium mb-1">Variante A</div>
