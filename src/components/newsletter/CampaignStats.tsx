...
export function CampaignStats({ campaignId, onBack }: Props) {
  ...
  if (loading || !campaign) return (
    <div className="flex items-center justify-center h-full"><Loader2 size={24} className="animate-spin text-p-accent" /></div>
  )

  const kpis = [
    { label: 'Taux d\'ouverture', value: `${openRate.toFixed(1)}%`, sub: `${campaign.unique_open_count} / ${campaign.delivered_count}`, icon: Eye, color: 'var(--c-accent)' },
  ...
  return (
    <div className="p-6 overflow-auto">
      <div className="flex items-center gap-3 mb-6">
        <button onClick={onBack} className="p-2 rounded-lg hover:bg-p-surface text-[#888] hover:text-[#F5F5F5]"><ArrowLeft size={18} /></button>
        <div>
        ...
        <div className="bg-p-surface border border-p-border rounded-xl p-5">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={hourData}>
              ...
              <Tooltip contentStyle={{ background: 'var(--c-surface)', border: '1px solid var(--c-surface)', borderRadius: 8, fontSize: 12 }} />
      ...
      {campaign.ab_test_enabled && (
        <div className="mt-6 bg-p-surface border border-p-border rounded-xl p-5">
          <h3 className="text-sm font-medium text-[#F5F5F5] mb-4">Résultats A/B Test</h3>
          <div className="grid grid-cols-2 gap-4">
            <div className="p-4 rounded-lg bg-p-surface/30">
              <div className="text-xs text-p-accent font-medium mb-1">Variante A</div>
