...
export function CampaignEditor({ campaignId, onBack }: Props) {
  ...
  const [accentColor, setAccentColor] = useState('var(--c-accent)')
  ...
  if (!campaign) return (
    <div className="flex-1 flex items-center justify-center bg-p-surface">
      <Loader2 size={24} className="animate-spin text-p-accent" />
    </div>
  )

  return (
    <div className="flex h-full bg-p-surface overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Toolbar */}
        <div className="flex items-center gap-2 px-4 py-2 bg-p-surface border-b border-p-border">
          <button onClick={onBack} className="p-1.5 rounded hover:bg-p-surface text-[#888] hover:text-[#F5F5F5] transition-colors"><ArrowLeft size={16} /></button>
          ...
          <div className="flex gap-1 ml-auto">
            {([
              ...
              <button key={mode} onClick={() => setViewMode(mode)}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs transition-colors ${viewMode === mode ? 'bg-p-accent text-black font-medium' : 'text-[#888] hover:text-[#F5F5F5] hover:bg-p-surface'}`}
              >
                <Icon size={13} />{label}
              </button>
            ))}
          ...
          <div className="flex gap-2 ml-4">
            <button onClick={handleSendTest} className="flex items-center gap-1.5 px-3 py-1.5 text-xs border border-p-border text-[#F5F5F5] rounded-md hover:bg-p-surface transition-colors"><TestTube2 size={13} />Tester</button>
            <button onClick={saveCampaign} disabled={isSaving} className="flex items-center gap-1.5 px-3 py-1.5 text-xs border border-p-border text-[#F5F5F5] rounded-md hover:bg-p-surface transition-colors">
              <Save size={13} />{isSaving ? 'Sauvegarde...' : 'Sauvegarder'}
            </button>
            <button className="flex items-center gap-1.5 px-3 py-1.5 text-xs bg-p-accent text-black rounded-md font-medium hover:bg-[#C47E00] transition-colors"><Send size={13} />Envoyer</button>
          </div>
        </div>

        {/* Canvas */}
        <div className="flex-1 overflow-auto p-6 flex justify-center bg-p-surface">
