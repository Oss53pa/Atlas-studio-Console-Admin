...
export function SubscribersList() {
  ...
  const kpis = [
    { label: 'Total', value: stats.total, icon: Users, color: '#A9B57E' },
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-4 gap-3 mb-6">
        {kpis.map(k => (
          <div key={k.label} className="bg-[#1c1c20] border border-[#2a2a30] rounded-xl p-4">
      ...
      <div className="flex items-center gap-3 mb-4">
        <div className="flex-1 relative">
          ...
          <input value={search} onChange={e => { setSearch(e.target.value); fetchSubscribers(e.target.value, statusFilter) }}
            placeholder="Rechercher..." className="w-full pl-9 pr-3 py-2 bg-[#1c1c20] border border-[#2a2a30] rounded-lg text-sm text-[#F5F5F5] outline-none focus:border-[#A9B57E]" />
        </div>
        <select value={statusFilter} onChange={e => { setStatusFilter(e.target.value); fetchSubscribers(search, e.target.value) }}
          className="px-3 py-2 bg-[#1c1c20] border border-[#2a2a30] rounded-lg text-sm text-[#F5F5F5] outline-none">
        ...
        <button onClick={() => setShowAdd(true)} className="flex items-center gap-1.5 px-3 py-2 bg-[#A9B57E] text-black rounded-lg text-sm font-medium hover:bg-[#C47E00]"><Plus size={14} />Ajouter</button>
        <button onClick={() => setShowImport(true)} className="flex items-center gap-1.5 px-3 py-2 border border-[#2a2a30] text-[#F5F5F5] rounded-lg text-sm hover:bg-[#2a2a30]"><Upload size={14} />Importer CSV</button>
        <button onClick={handleExport} className="flex items-center gap-1.5 px-3 py-2 border border-[#2a2a30] text-[#F5F5F5] rounded-lg text-sm hover:bg-[#2a2a30]"><Download size={14} />Exporter</button>
      </div>

      {/* Add form inline */}
      {showAdd && (
        <div className="mb-4 p-4 bg-[#1c1c20] border border-[#A9B57E]/30 rounded-xl flex items-end gap-3">
          <div className="flex-1">
            <label className="text-[11px] text-[#888] mb-1 block">Email</label>
            <input value={newEmail} onChange={e => setNewEmail(e.target.value)} placeholder="email@example.com" className="w-full px-3 py-1.5 bg-[#2a2a30] rounded text-sm text-[#F5F5F5] outline-none" />
          </div>
          <div className="flex-1">
            <label className="text-[11px] text-[#888] mb-1 block">Nom (optionnel)</label>
            <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="Nom complet" className="w-full px-3 py-1.5 bg-[#2a2a30] rounded text-sm text-[#F5F5F5] outline-none" />
          </div>
          <button onClick={handleAdd} className="px-4 py-1.5 bg-[#A9B57E] text-black rounded text-sm font-medium">Ajouter</button>
          <button onClick={() => setShowAdd(false)} className="px-4 py-1.5 text-[#888] text-sm">Annuler</button>
        </div>
      )}

      {/* Table */}
      {loading ? <div className="flex justify-center py-20"><Loader2 size={20} className="animate-spin text-[#A9B57E]" /></div> : (
        <div className="bg-[#1c1c20] border border-[#2a2a30] rounded-xl overflow-hidden">
          <table className="w-full">
            ...
            <tbody>
              {filtered.map(s => (
                <tr key={s.id} className="border-b border-[#2a2a30]/50 hover:bg-[#2a2a30]/20">
